<template>
  <div
    ref="containerRef"
    :class="`relative w-125 ${className}`.trim()"
  >
    <div
      ref="listRef"
      :class="`max-h-100 overflow-y-auto p-4 ${
        displayScrollbar
          ? '[&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-[#0b0b0b] [&::-webkit-scrollbar-thumb]:bg-[#222] [&::-webkit-scrollbar-thumb]:rounded-[4px]'
          : 'scrollbar-hide'
      }`"
      :style="{
    ...
    >
      <Motion
        ...
        @click="() => { setSelectedIndex(index); emit('itemSelected', item, index) }"
      >
    ...
    <div
      v-if="showGradients"
      class="absolute top-0 left-0 right-0 h-12.5 bg-linear-to-b from-[#0b0b0b] to-transparent pointer-events-none transition-opacity duration-300 ease"
      :style="{ opacity: topGradientOpacity }"
    />
    <div
      v-if="showGradients"
      class="absolute bottom-0 left-0 right-0 h-25 bg-linear-to-t from-[#0b0b0b] to-transparent pointer-events-none transition-opacity duration-300 ease"
