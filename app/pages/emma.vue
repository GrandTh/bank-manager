...
useSeoMeta({ title: 'Emma · Budget' })

const store = useTransactionsStore()

const revenus = computed(() => store.totalCredits('emma'))
const depenses = computed(() => store.totalDebits('emma'))
const solde = computed(() => revenus.value - depenses.value)

const prevMonth = computed(() =>
  store.previousYearMonth ? store.totalsForMonth('emma', store.previousYearMonth) : null
)
...
const epargne = computed(() =>
  epargneCategory.value ? (store.byCategory('emma')[epargneCategory.value.key] ?? 0) : 0
)
...
const hasData = computed(() => store.filteredFor('emma').length > 0)
const emmaBudgets = computed(() => store.budgets.filter(b => b.person === 'emma'))
const emmaGoals = computed(() => store.goals.filter(g => g.scope === 'commun' || g.scope === 'emma'))
const emmaAlerts = computed(() => store.budgetAlerts('emma'))
</script>
...
<template>
  <div class="flex flex-col p-6 gap-6">
    ...
    >
      ...
      />
      <p class="text-sm text-muted">
        Aucune transaction — importez un relevé CSV pour Emma.
      </p>
    </div>
      ...
      <div class="flex gap-4 h-[calc(100dvh-240px)]">
        ...
        <div class="flex flex-col gap-4 w-1/2">
          <div class="rounded-xl border border-default bg-default p-4 flex-1 min-h-0">
            ...
            <ClientOnly><ChartsSpendingDonut person="emma" /></ClientOnly>
          </div>
          <div class="rounded-xl border border-default bg-default p-4 flex-1 min-h-0">
            <h3 class="text-sm font-semibold text-default mb-4">
        ...
        <div class="flex-1 overflow-hidden rounded-xl border border-default bg-default flex flex-col">
          ...
          <AnimatedTransactionList person="emma" />
        </div>
      </div>
        ...
        <div class="rounded-xl border border-default bg-default p-4">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
              ...
              <UBadge
                v-if="emmaAlerts.over > 0"
                :label="`${emmaAlerts.over} dépassé${emmaAlerts.over > 1 ? 's' : ''}`"
                color="error"
                variant="subtle"
                size="xs"
              />
              <UBadge
                v-else-if="emmaAlerts.warning > 0"
                :label="`${emmaAlerts.warning} proche${emmaAlerts.warning > 1 ? 's' : ''}`"
                color="warning"
                variant="subtle"
                size="xs"
              />
            </div>
            <BudgetsModal person="emma" />
          </div>
          <div
            v-if="emmaBudgets.length"
            class="space-y-3"
          >
            <BudgetProgressBar
              v-for="b in emmaBudgets"
              :key="b.id"
              person="emma"
